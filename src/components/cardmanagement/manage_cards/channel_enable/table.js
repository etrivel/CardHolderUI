...
class ManageChannelTable extends React.Component {
  // state
  constructor(props) {
    super(props);
    this.state = {
  ...
  //     pflag: "s",
  //     reqMsgId: "",
  //     vpan: "PMMG000000000012",
  //   };
  //   this.apiFetch("api/v1/enquiry/carddetails", body);
  // };

  handledata = async (val) => {
    this.context.setData({ ...this.context, channel: val });
    this.props.history.push("/limit_Management");
  };
  fetchTable = () => { };

  apiFetch = async (api, playod) => {
    await axios
...
ManageChannelTable.contextType = EdittablesContext;
export default withStyles(styles)(withRouter(ManageChannelTable));
